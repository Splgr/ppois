...
# Импорт моделей
from .models import (
    Room, Guest, Booking, Period, ServiceOrder, 
    Employee, RoomStatus, BookingStatus, ServiceType
)
class HotelStorage:
    ...
    def _load_or_initialize(self):
        if self._filepath.exists():
            try:
                ...
                print(f"Данные успешно загружены из {self._filepath}")
                return
            except Exception as e:
                print(f"Ошибка загрузки: {e}. Создаём новую базу...")
                self._filepath.unlink(missing_ok=True)
        
        # Только если файла нет — создаём демо-данные
        self._initialize_demo_data()
        self.save_to_file()
        print(f"Создан новый файл с демо-данными: {self._filepath}")
