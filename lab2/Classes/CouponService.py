...
class CouponService:
    def validate_and_apply(self, coupon: Coupon, order_amount: float) -> float:
    ...
    def create_percentage_coupon(self, discount_percent: float, days_valid: int = 30) -> PercentageCoupon:
        expiry_date = datetime.now() + timedelta(days=days_valid)
        return PercentageCoupon(discount_percent, expiry_date)
    
    def create_fixed_coupon(self, discount_amount: float, min_order_amount: float = 0, days_valid: int = 30) -> FixedAmountCoupon:
        expiry_date = datetime.now() + timedelta(days=days_valid)
