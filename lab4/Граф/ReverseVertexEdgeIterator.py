class ReverseVertexEdgeIterator:
    """Обратный итератор для инцидентных рёбер"""
    ...
    def prev(self):
        if self._current_index >= len(self._neighbors) - 1:
            raise StopIteration
        self._current_index += 1
        neighbor = self._neighbors[self._current_index]
        u, v = min(self._vertex, neighbor), max(self._vertex, neighbor)
    ...
    def __eq__(self, other):
        if not isinstance(other, ReverseVertexEdgeIterator):
            return False
